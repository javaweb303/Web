					...
					<div class="selectNomal">
						<div class="selectBox01Nomal">
							<select name="searchCa" class="select width nomal" onchange="location.href=(this.value)">
								<option value="/recomm?searchCa=0" <c:if test="${categoryId==0}">selected</c:if> >전체</option>
								...
								<li>
									<!-- <div class="book"> -->
										<div class="over" onclick="location.href='/bookcont?isbn=${list['isbn13']}';">
											<strong>${list['title']}</strong><br> <span>${list['authors']}</span>
											<p>${list['publisher']}</p>
											<p>${list['pubDate']}</p>
										</div> <img alt="책이미지" src="${list['cover']}"
