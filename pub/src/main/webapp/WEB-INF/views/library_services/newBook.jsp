		...
		<div class="cont_wrap">
			...
			<div class="cont_box">
				...
				<div>
					...
					<form name="search" method="post" action="/new">
					<div class="selectNomal">
						<div class="selectBox01Nomal">
							<select name="searchCa" class="select width nomal" onchange="location.href=(this.value)">
								<option value="/new?searchCa=0" <c:if test="${categoryId==0}">selected</c:if> >전체</option>
								<option value="/new?searchCa=1" <c:if test="${categoryId==1}">selected</c:if>>소설/시/희곡</option>
								<option value="/new?searchCa=656" <c:if test="${categoryId==656}">selected</c:if>>인문학</option>
								<option value="/new?searchCa=987" <c:if test="${categoryId==987}">selected</c:if>>과학</option>
								<option value="/new?searchCa=798" <c:if test="${categoryId==798}">selected</c:if>>사회과학</option>
								<option value="/new?searchCa=74" <c:if test="${categoryId==74}">selected</c:if>>역사</option>
								<option value="/new?searchCa=517" <c:if test="${categoryId==517}">selected</c:if>>예술/대중문화</option>
								<option value="/new?searchCa=1322" <c:if test="${categoryId==1322}">selected</c:if>>외국어</option>
								<option value="/new?searchCa=1237" <c:if test="${categoryId==1237}">selected</c:if>>종교/역학</option>
							</select>
						</div>
					</div>
					</form>
